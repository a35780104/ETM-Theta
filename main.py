...
def visualize(m, show_emb=True):
    ...
    # queries = ['service', 'computer', 'client', 'religion', 'man', 'love', 
...
def no_test_get_data(path):
    ...
    train = no_test_fetch(path, 'train')
...
vocab_size = len(v)
no_test_num_docs_train = len(train_tokens)
num_docs_train = train_tokens


thetas = get_document_topic_dist(model, device, vocab_size, no_test_num_docs_train, args.batch_size)
print(len(thetas), no_test_num_docs_train)
print(type(thetas.cpu().numpy()))
np.save('topic_doc_theta.npy',thetas.cpu())
